...
- The main workspace includes a simulation tab that can choose a tagged meta deck as the default matchup, configure player count, each player's deck, human or AI roles, opponent hand visibility, play speed, and board zoom, generate a deterministic opening simulation from a seed, show image-based player zones for hand, battlefield, graveyard, exile, and keep logs at the bottom.
...
- Simulation player settings include a deck selector per player; the current loaded deck can be assigned to any player, and tagged meta sessions can be assigned independently per player while falling back to the selected matchup deck for unset opponents.
