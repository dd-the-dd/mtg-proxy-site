...
- Card Analysis and Play share the Magic rule language documented in `feature_files/magic-rule-language.feature.md`, including color-coded actions, reads, predicates, references, costs, triggers, timing, Magic vocabulary, and effect blocks.
...
- Oracle analysis first detects higher-level concepts such as hooks, boolean logic, targets, concrete actions, and choices before parsing each concept's detailed parameters, so future mechanics can extend one detector without duplicating low-level token handling.
- Oracle target detection resolves self-references made with `this`, the full source card name, or a leading prefix of the source card name when the phrase clearly points back to the source card.
...
- Oracle analysis merges activated exile-play abilities where an activation exiles the top library card and the next sentence grants permission to play that card until the end of the next turn; the merged option exposes the activation costs, the library-to-exile move, the temporary exile play permission, and the scheduled permission cleanup.
- Oracle analysis maps tap mana abilities such as `{T}: Add {R}.` to a battlefield option with a tap cost, source-on-battlefield and source-untapped conditions, and an add-mana action; land play from hand remains a separate default land option instead of being shown as the mana ability's option.
- Oracle analysis classifies stack-protection text such as `This spell can't be countered.` as a stack-object modifier, and activated abilities such as `The next spell you cast this turn can't be countered` as a delayed cast hook that modifies the next spell on the stack.
- Oracle analysis merges `Choose one` modal openers with their bullet choices, exposes the player modal choice, and keeps each choice's own resolution action and target requirement.
