...
- Card Analysis shows each current deck card with its image, parser coverage, parser gaps, parsed hooks, option resolution actions, and whether each hook is engine-ready, stack-only, parser-only, or unsupported so parser progress and simulation gaps are visible in the UI.
