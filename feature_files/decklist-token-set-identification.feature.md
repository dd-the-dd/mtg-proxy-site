...
- Simulation board state separates creatures, lands, and noncreature permanents, stacks identical permanents that share the same state with a quantity badge, lets graveyard and exile stacks expand, and surfaces graveyard or exile cards that can be reused as virtual hand cards.
- Simulation board layout shows empty graveyard and exile placeholders, places two-player games face-to-face with the opponent hand/zones/resources/creatures above the current player's creatures/resources/hand/zones, and places additional player pairs in horizontally scrollable lanes that alternate the resource mirror side.
