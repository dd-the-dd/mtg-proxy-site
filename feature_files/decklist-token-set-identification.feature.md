...
- Oracle parsing classifies `This <entity> enters tapped unless you control <entity> or <entity>` as an enter-battlefield state modifier on the source card, with `unless` represented as an inverted `youControlAny` condition over flexible type, subtype, card-name, or unresolved identifier candidates.
