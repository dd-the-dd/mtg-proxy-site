...
- The main workspace includes a simulation tab that can choose a tagged meta deck as opponent, configure player count, human or AI roles, opponent hand visibility, play speed, and board zoom, generate a deterministic opening simulation from a seed, show image-based player zones for hand, battlefield, graveyard, exile, and keep logs at the bottom.
...
- Simulation board layout keeps the phase controls, life totals, hand row, and battlefield compact enough for a two-player game to remain visible in one viewport; hand cards and zone stacks can be smaller than battlefield permanents so the in-play cards remain readable.
...
- Simulation logs include resolved player and AI actions, including the step, player, action label, source card, and target when present.
...
- Simulation board layout shows empty graveyard and exile placeholders without using a Magic card back, places two-player games face-to-face with the opponent hand/zones/battlefield above the current player's battlefield/hand/zones, compacts two-player games so the defined zones fit in the main viewport, and places additional player pairs in horizontally scrollable lanes.
