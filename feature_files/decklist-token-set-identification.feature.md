...
- Value analysis for damage-based removal spells, such as Abrade, shows per-meta-deck battlefield removal coverage under the cast option, including removed and affected percentages over the full meta-deck card quantity plus collapsible target details and protection responses.
...
- Value analysis includes colorless sources that can pay generic or colorless costs, keeps land mana sources condensed as land-colored chips, and renders artifacts or mana-burst spells that can pay a cast option as normal payment rows.
