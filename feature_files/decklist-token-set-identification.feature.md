...
- Resolving mill moves cards from the top of that player's library into their graveyard, while scry reveals the top card choices and lets the player keep cards on top or put selected cards on the bottom of that library.
...
- Card Analysis shows each current deck card with its image, parser coverage, and an official-oracle analysis list where the oracle text is split into sentence-like segments; each segment carries hook, option, or unsupported annotations through row color and badges instead of separate per-card hook and option panels.
- Card Analysis displays each parsed hook as a rule with a game timing, a parenthesized activation logic expression using `not`, `and`, `or`, named condition calls, and an ordered action list; every action row can expose its own activation logic, cost, state operation, and Python-like action definition that mutates game state and returns `None`.
- Card Analysis keeps an always-visible parser feedback textbox on every oracle segment, including parsed segments, unsupported segments, and merged multi-sentence segments.
- Card Analysis lists possible card options with their required source zone, costs and conditions to put the spell or ability on the stack, target requirements that must be valid before costs are paid, and the later resolution behavior including target-fizzle handling.
- Card Analysis stores parser feedback comments for unsupported clauses and inferred player options in a local file-backed feedback store, and can show hook/event, condition, action, and player-option contracts with pseudo-code definitions and parameters.
- Oracle analysis parsing is modeled as a word-by-word state machine; an unexpected token or unsupported state produces an unparsable diagnostic with parser mode, parser state, and the unexpected token for audit.
- Oracle analysis may merge adjacent official sentences into a single annotated segment when one hook or option spans both sentences, such as a trigger followed by a rider sentence.
- Oracle parsing classifies `This <entity> enters tapped unless you control <entity> or <entity>` as an enter-battlefield hook on the source card, with one `entersTapped` branch gated by `not(youControlAny(...))` and one `entersUntapped` branch gated by `youControlAny(...)` over flexible type, subtype, card-name, or unresolved identifier candidates.
...
- Card-trigger hooks are represented as serializable rules with a source card, source controller, hook event, named condition-expression tree, and ordered action steps; conditions are pure boolean getters over event and game state, while actions mutate game state and can carry their own cost or activation logic.
