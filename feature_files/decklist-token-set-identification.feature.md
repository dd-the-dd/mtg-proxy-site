...
- Card Analysis shows each current deck card with its image, parser coverage, parsed Oracle actions, rule-trigger hooks, and whether each hook is engine-ready, stack-only, parser-only, or unsupported so parser progress and simulation gaps are visible in the UI.
