...
- The main workspace includes a Play setup screen that configures player count, each player's deck, human or AI roles, opponent hand visibility, play speed, and board zoom before the board is shown. Player one defaults to the current loaded deck when Play is opened from a deck, and other players choose directly from tagged meta decks instead of inheriting a separate matchup field.
...
- Simulation player settings keep player one on the current loaded deck by default and include deck selectors for additional players using tagged meta sessions. The Play setup does not expose a separate matchup field or a turn-count field, and unset additional players do not inherit a hidden matchup fallback.
