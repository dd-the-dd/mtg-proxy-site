...
describe('Core Rendering', async () => {
    ...
    test('Feature: Simulation mill actions move the top library card into graveyard.', () => {
        const component = wrapper.getCurrentComponent();
        const openingCard = {
            id: 'mountain:0',
            name: 'mountain',
        ...
        expect(player.zones.libraryCount).toBe(1);
        expect(player.zones.graveyard.count).toBe(1);
        expect(player.zones.graveyard.cards).toContainEqual(expect.objectContaining({
            name: 'opt',
            quantity: 1,
        }));
        expect(player.zones.graveyard.top).toMatchObject({
            name: 'opt',
        });
        expect(player.library).toEqual([openingCard, secondLibraryCard]);
    });

    test('Feature: Simulation scry actions keep or put top cards on the bottom of library.', () => {
        const component = wrapper.getCurrentComponent();
        const openingCard = {
            id: 'mountain:0',
            name: 'mountain',
        ...
        expect(player.zones.libraryCount).toBe(2);
        expect(player.library).toEqual([openingCard, topLibraryCard, secondLibraryCard]);
        ...
        expect(player.zones.libraryCount).toBe(2);
        expect(player.library).toEqual([openingCard, secondLibraryCard, topLibraryCard]);
        expect(player.zones.graveyard.count).toBe(0);
        expect(player.zones.hand).toEqual([openingCard]);
    });
